...
enum abstract {{global.classname}}Obj(Int) {
{% for body in bodies %}
	var {{body.name}};{% endfor %}
...
class {{global.classname}} {
	 ...
	 */
	public static inline function createBody(name:{{global.classname}}Obj):Body {
		return lookup(name).body.copy();
	}
	 ...
	 * @return Vec2
	 */
	public static inline function getAnchor(name:{{global.classname}}Obj):Vec2 {
		return lookup(name).anchor;
	}
	 ...
	 */
	public static inline function registerMaterial(name:String, material:Material):Void {
		if (materials == null) materials = new StringMap<Material>();
	 ...
	 */
	public static inline function registerFilter(name:String, filter:InteractionFilter):Void {
		if (filters == null) filters = new StringMap<InteractionFilter>();
	 ...
	 */
	public static inline function registerFluidProperties(name:String, properties:FluidProperties):Void {
		if (fprops == null) fprops = new StringMap<FluidProperties>();
	 ...
	 */
	public static inline function registerCbType(name:String, cbType:CbType):Void {
		if (types == null) types = new StringMap<CbType>();
		types.set(name, cbType);
	}

	//----------------------------------------------------------------------

	static var bodies:Map<{{global.classname}}Obj, {body:Body,anchor:Vec2}>;
	...
	static inline function material(name:String):Material {
		return if (name == "default")
			new Material();
		else if (materials==null || !materials.exists(name))
	...
	static inline function filter(name:String):InteractionFilter {
		...
		else if (filters==null || !filters.exists(name))
	...
	static inline function fprop(name:String):FluidProperties {
		return if (name=="default")
			new FluidProperties();
		else if (fprops==null || !fprops.exists(name))
			throw "Error: FluidProperties with name '" + name + "' has not been registered";
		else
			fprops.get(name);
	}

	static inline function cbtype(outtypes:CbTypeList, names:String):Void {
	...
	static inline function lookup(name:{{global.classname}}Obj):{body:Body, anchor:Vec2} {
		if (bodies == null) init();
		if (!bodies.exists(name))
			throw "Error: Body with name '" + name + "' does not exist";
	...
	static function init():Void {
		bodies = new Map<{{global.classname}}Obj, {body:Body, anchor:Vec2}>();
		{% for body in bodies %}
			var body = new Body();
